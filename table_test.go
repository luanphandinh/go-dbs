...
func TestToTableDeclaration(t *testing.T) {
	...
	assertStringEquals(t, "CREATE TABLE IF NOT EXISTS public.user (id INT NOT NULL, name TEXT NOT NULL, age INT DEFAULT 10 CHECK (age < 1000), PRIMARY KEY (id), CHECK (age > 50)); CREATE SEQUENCE IF NOT EXISTS public.user_id_seq; ALTER TABLE public.user ALTER id SET DEFAULT NEXTVAL('public.user_id_seq');", postgresPlatform.GetTableCreateSQL("public", &table))
